...
def get_all_subscribers():
    """Get all unique emails from subscribers table."""
    try:
        ...
        data = response.json()
        emails = list(set(item["email"] for item in data))
...
def get_subscriber_listings(email):
    """Get all bookmarked listings for a subscriber email."""
    try:
        response = requests.get(
            f"{SUPABASE_URL}/rest/v1/subscribers?email=eq.{email}&select=listing_id",
            headers={
                "apikey": SUPABASE_SERVICE_KEY,
                "Authorization": f"Bearer {SUPABASE_SERVICE_KEY}"
            }
        ...
        print(f"Error fetching listings for {email}: {e}")
        return []
...
def build_email_html(listings, email_type="weekly"):
    ...
    def card(l):
        ...
        link = l.get("link", "")
        type_labels = {"phd": "PhD", "msc": "MSc", "postdoc": "Postdoc", "paper": "Paper Call", "grant": "Grant", "conf": "Conference"}
        ...
        <div style="background:#ffffff;border:1px solid #e8e0d4;border-radius:10px;padding:18px 20px;margin-bottom:12px;">
          ...
          <div style="font-family:Georgia,serif;font-size:16px;font-weight:600;color:#1a1612;margin:10px 0 4px;">{l.get("title","")}</div>
    ...
    if urgent:
        ...
        <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:14px 18px;margin-bottom:20px;">
          <div style="font-size:14px;font-weight:600;color:#dc2626;">🚨 Urgent — deadline in 5 days or less</div>
    ...
    greeting = "Here is your weekly digest" if email_type == "weekly" else "Here is your urgent deadline alert"
      ...
      <div style="max-width:600px;margin:0 auto;padding:32px 16px;">
        ...
        <div style="background:#ffffff;padding:28px;border-radius:0 0 12px 12px;margin-bottom:20px;">
          ...
          <div style="font-size:14px;color:#6b6358;line-height:1.7;margin-bottom:24px;">
            {greeting} from Alaye. You have <strong>{len(listings)}</strong> saved opportunit{"ies" if len(listings) != 1 else "y"}.
            {"Don't let deadlines slip by!" if urgent else "Keep track of your applications and stay ahead."}
          </div>
          {urgent_section}
          {upcoming_section}
          {rest_section}
...
def send_email(to_email, subject, html):
    ...
    try:
        response = requests.post(
            ...
            json={
                "from": "Alaye <hello@alaye-agent.live>",
                "to": [to_email],
                "subject": subject,
                "html": html
            }
